function [R, dR_dx,J ] = Shape_function_repoint( GP,e,deg,B,KV,INC,IEN)
...
if (e>=((n-p)*(m-q)+1)&&e<=((n-p)*(m-q+1)))||(e>=((n-p)*(m-q)+1+(n-p)*(m-q+1))&&e<=((n-p)*(m-q+1)*2))
 nj = size(B,2)+1;   
else
nj = INC(IEN(1,e),2);
end
...
loc_num = 0; % Local basis function counter
 for k = 0 : r
    for j = 0 : q
        for i = 0 : p
            loc_num = loc_num + 1;
          if ((e>=((n-p)*(m-q)+1)&&e<=((n-p)*(m-q+1)))||(e>=((n-p)*(m-q)+1+(n-p)*(m-q+1))&&e<=((n-p)*(m-q+1)*2)))&&j==0
            R(loc_num) = N(p+1-i)*M(q+1-j)*L(r+1-k) ...
                        * B{ni-i,1,nk-k}(4); % Function numerator (N*M*L*w) 
            x(loc_num) = B{ni-i,1,nk-k}(1) ;
            y(loc_num) = B{ni-i,1,nk-k}(2);
            z(loc_num) = B{ni-i,1,nk-k}(3);
             dR_dxi(loc_num,1) = dN_dxi(p+1-i)*M(q+1-j)*L(r+1-k) ...
           * B{ni-i,1,nk-k}(4); % Derivative numerator (dN*M*L*w)
        
            dR_dxi(loc_num,2) = N(p+1-i)*dM_deta(q+1-j)*L(r+1-k) ...
            * B{ni-i,1,nk-k}(4); % Derivative numerator (N*dM*L*w)


           dR_dxi(loc_num,3) = N(p+1-i)*M(q+1-j)*dL_dzeta(r+1-k) ...
           * B{ni-i,1,nk-k}(4); % Derivative numerator (N*M*dL*w)
            else
           R(loc_num) = N(p+1-i)*M(q+1-j)*L(r+1-k) ...
