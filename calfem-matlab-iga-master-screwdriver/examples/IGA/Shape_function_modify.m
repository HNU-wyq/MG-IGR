function [ Bm,J,R ,T] = Shape_function_modify( GP,e,deg,B,KV,INC,IEN)
...
dR_dxi(:,1) = (dR_dxi(:,1)*W - R*dW_dxi) / W^2   ;      %R,1
dR_dxi(:,2) = (dR_dxi(:,2)*W - R*dW_deta) / W^2 ;        %R,2
...
%%-----------------------5个自由度的mindlin壳---------------------------------------------------------
 Bm=zeros(3,nen*3); Bb=zeros(3,nen*3);
ti=0.1;
%  Bm=zeros(6,nen*5);
...
Jss=zeros(3);
      ...
      dS_ds=[x;y;z] * dR_dxi(:,1) ;  %G1
      dS_dt=[x;y;z] * dR_dxi(:,2);   %G2
      c= cross(dS_ds,dS_dt);
%       
%       J1=norm(c);
%       J2=((KV.Xi(ni+1)-KV.Xi(ni))/2)*(KV.Eta(nj+1)-KV.Eta(nj))/2;
%        J=J1*J2;   
      v3=c/norm(c);
      norm(v3)
...
dS_ss=-2*W*dW_dxi*(dR_dxi(:,1)*W - R*dW_dxi)/ W^4;              %R,11
front=(dR_dxdeta*W+dW_deta*dR_dxi(:,1)-dR_dxi(:,2)*dW_dxi-R* dW_dxdeta)*W^2;
dS_st=(front-2*W*dW_deta*(dR_dxi(:,1)*W - R*dW_dxi)) / W^4;     %R,12       
dS_tt=-2*W*dW_deta*(dR_dxi(:,1)*W - R*dW_dxi)/ W^4;             %R,22
  
dS_dss=[x;y;z]*dS_ss;  %G1,1
dS_dst=[x;y;z]*dS_st;   %G1,2 
dS_dtt=[x;y;z]*dS_tt;   %G2,2 

%{
e=[e1,e2,e3];
for lo=1:loc_num
 Bmi=zeros(3); Bbi=zeros(3);
 p11=-dS_ss(lo)* v3; p12=-dS_tt(lo)* v3; p13=-dS_st(lo)* v3;
 p2=1/sqrt(J);
 p31=cross(dR_dxi(lo,1)*dS_dss, dS_dt)+cross(dR_dxi(lo,2)*dS_ds, dS_dss);
 p4=cross(dR_dxi(lo,1)*dS_dt, v3)+cross(dR_dxi(lo,2)*v3, dS_ds);
 p32=cross(dR_dxi(lo,1)*dS_dtt, dS_dt)+cross(dR_dxi(lo,2)*dS_ds, dS_dtt);
 p33=cross(dR_dxi(lo,1)*dS_dst, dS_dt)+cross(dR_dxi(lo,2)*dS_ds, dS_dst);
 
 Bb1 = p11+p2*p31+ dot(v3,dS_dss)*p2* p4;
 Bb2 = p12+p2*p32+dot(v3,dS_dtt)*p2* p4;
 Bb3 = p13+p2*p33+dot(v3,dS_dst)*p2* p4;
 
 for i=1:length(Bmi)
     Bmi(1,i)=dot(dR_dxi(lo,1)*dS_ds,e(:,i));
     Bmi(2,i)=dot(dR_dxi(lo,2)*dS_dt,e(:,i));
     Bmi(3,i)=dot((dR_dxi(lo,1)*dS_dt+dR_dxi(lo,2)*dS_ds),e(:,i));
     
     Bbi(1,i)=dot(Bb1,e(:,i));
     Bbi(2,i)=dot(Bb2,e(:,i));
     Bbi(3,i)=2*dot(Bb3,e(:,i));
     
 end
 Bm(1:3,3*(lo-1)+1:3*lo)= Bmi;
 Bb(1:3,3*(lo-1)+1:3*lo)= Bbi;
end
%}
