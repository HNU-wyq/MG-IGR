...
% solid11=h_refinement(solid11.coefs,solid11.knots);
...
% [B,Zeta]=Re-1.000000	-2.750000	0.000000fineKnotSolidZeta( B,deg.r,Zeta,Z_ );
solid11=h_refinement(solid11.coefs,solid11.knots,deg);
solid12=h_refinement(solid12.coefs,solid12.knots,deg);
...
B=solid12.coefs;BJ=cell2mat(B);
Xi = solid12.knots{1};
Eta = solid12.knots{2};
Zeta= solid12.knots{3};
...
for q = 1 : nel
     ai(:,q)=a(LM(:,q)) ;
      ...
      ni = INN(IEN(1,q),1); nj = INN(IEN(1,q),2);nk = INN(IEN(1,q),3);
    ...
    for i = 1 : NQUADx % Loop trough Gauss points
        for j = 1 : NQUADy
            for k = 1 : NQUADz
                ...
                siga=D*strain(:,gp,q);
                stress(gp,q,1:3)=siga(1:3);
          % von Mises stress
                stress(gp,q,4)  = sqrt(siga(1)^2+siga(2)^2-...
                                   siga(1)*siga(2)+3*siga(3)^2); 
                stress_n(IEN(gp,q),:)=stress(gp,q,:);
...
%{
%------------环形模型需增加的的应力操作---------
 mise=zeros(21*3*25,4);ip=size(B,2)*size(B,3);
 for i=1:ip
     mise((21*(i-1)+1):(21*i-1),:)=stress_n((20*(i-1)+1):20*i,:);
     mise(21*i,:)=stress_n(20*(i-1)+1,:);
 end
%}
            ...
            for i = 1 : n1
                ...
                if i >= (1+1) & j >= (1+1) & k >= (1+1)
                    ...
                    for kloc = 0 : 1
                        for jloc = 0 : 1
                            for iloc = 0 : 1
                                B_ = A - kloc*n1*m1- jloc*n1 - iloc; % global function number
