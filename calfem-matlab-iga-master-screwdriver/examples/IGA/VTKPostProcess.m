...
function VTKPostProcess(node,elementV,etype,vtuFile,sigma,displacement)
...
if(strcmp(etype, 'Quad4') || strcmp(etype, 'Quad8') || strcmp(etype, 'Quad9'))  %平面四边形
    numVertexesPerCell = 4;
    VTKCellCode = 9;
elseif(strcmp(etype, 'B8'))   
    numVertexesPerCell = 8;
    VTKCellCode = 12;
elseif(strcmp(etype, 'Tri3') || strcmp(etype, 'Tri6'))  %平面三角形
    numVertexesPerCell = 3;
    VTKCellCode = 5;
elseif(strcmp(etype, 'Tet4'))   %四面体
    numVertexesPerCell = 4;
    VTKCellCode = 10;
else
...
dispX   = displacement(1:2:end);
dispY   = displacement(2:2:end);
...
for i=1:numNodes
    fprintf(results_vtu, '%g   ', sigmaXX(i) );
    fprintf(results_vtu, '%g   ', sigmaYY(i) );
    fprintf(results_vtu, '%g   ', sigmaXY(i) );
    
    if size(sigma,2)==4
      fprintf(results_vtu, '%g   ', sigmaVM(i) );
    end
...
for i=1:numNodes
    fprintf(results_vtu, '%.12f   ', dispX(i) );
    fprintf(results_vtu, '%.12f   ', dispY(i) );
    fprintf(results_vtu, '0.0' );
    fprintf(results_vtu, '\n');
end
