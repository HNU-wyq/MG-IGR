function [INBz1,INNz1,Br1,Bz11,Bz] =Midgridx(B,lb,ub,Ymax,Ymin,KV,deg) 
Bz=cell(size(B));
% for i=1:size(B,2)
%     for j=1:size(B,3)
%         Bz{1,i,j}(2:3)= (Ymax*ub(1))/norm(B{2,i,j}(2:3)).*B{2,i,j}(2:3);
%         Bz{3,i,j}(2:3)= (Ymin*lb(1))/norm(B{3,i,j}(2:3)).*B{3,i,j}(2:3);
%         Bz{2,i,j}(2:3)=( Bz{1,i,j}(2:3)+ Bz{3,i,j}(2:3))/2 ;
%          Bz{1,i,j}(4)= B{1,i,j}(4);  
%          Bz{2,i,j}(4)= B{2,i,j}(4);
%          Bz{3,i,j}(4)= B{3,i,j}(4);
%          Bz{1,i,j}(1)=B{1,i,j}(1);
%          Bz{2,i,j}(1)=B{2,i,j}(1);
%          Bz{3,i,j}(1)=B{3,i,j}(1);
%     end
% end
Bz=B;
for i=1:size(B,2)
...
[INNz1,IENz1,INBz1] = BLDINCIEN11( degz,size(Bz,1),size(Bz,2),size(Bz,3));
% figure (3)
