function output=error_objecfun(x,nVar,INBz1,INNz1,Br1,Bz11,Bz,nsub,ssub,idsub,Pp1)
%% Load data
...
%     end 
    ...
    for k=1:numel(nub)
     if  B{i,nub(k),1}(2)>=10e-10    
          ...
          B{i,nub(k),2}(2)=B{i,nub(k),2}(2)-x(1);             %刀头处优化为凸边形（8边形） 一个设计变量改变6组控制点 （0.8-1.8）
...
%%
ai=zeros(3*nen,nel);
num_node1=zeros(nnp,1);
stress_nE=zeros(nnp,4);
stressE=zeros(nen,nel,4);
for sq = 1:nel
     ai(:,sq)=aE(LM(:,sq)) ;
...
%     gp=1;np=1;
     for gp=1:size(IEN,1)
                ...
                kinmtx=Bmatrix{gp,sq};
                strainE(:,gp,sq)=kinmtx*ai(:,sq);
                siga=D*strainE(:,gp,sq);
                
                stressE(gp,sq,1:3)=siga(1:3);
          % von Mises stress
                stressE(gp,sq,4)  = sqrt(((siga(1)-siga(2))^2+(siga(3)-siga(2))^2+...
                                   (siga(1)-siga(3))^2)/2);
                                aaaE(1,:)=stressE(gp,sq,:);    
                stress_nE(IEN(gp,sq),:)=stress_nE(IEN(gp,sq),:)+aaaE(1,:);
    end
%                 num_node(IEN(gp,sq))= num_node(IEN(gp,sq))+1;
%                 gp=gp+1;
%             end
...
for jj=1:size(IEN,1)
   for ii=1:nel
        num_node1(IEN(jj,ii))= num_node1(IEN(jj,ii))+1;
    end
 end
%  for ii=1:n*m*l
%    
...
% 
% end
stress_n=bsxfun( @rdivide,stress_n,num_node1);
stress_nE=bsxfun( @rdivide,stress_nE,num_node1);
output=(max(stress_n(:,4))-max(stress_nE(:,4)))./max(stress_nE(:,4));
